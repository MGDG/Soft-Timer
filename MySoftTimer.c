...
/**
  ...
  */
uint32_t SOFT_TIMER_CreateTimer(const uint32_t interval, Function timproc,uint8_t on_off)
{
...
bool SOFT_TIMER_StartTimer(uint32_t index)
{
	...
	{
		if( ((SOFT_TIMER_LINK_NODE *)index)->enable  == false )
		{
...
{
	DEBUGOUT("handle1 timer running");
}
static void testhandle(void)
{
	...
	if(flag)
	{
		...
		{
			DEBUGOUT("handle1 timer start");
		}
		else
		{
			DEBUGOUT("handle1 timer start error");
		}
		flag = false;
	}
	else
	{
		...
		{
			DEBUGOUT("handle1 timer stop");
		}
		else
		{
			DEBUGOUT("handle1 timer stop error");
		}
		flag = true;
	}
}
bool softtimer_test_start(void)
{
	if(timertest_TimerIndex == 0)
	{
		...
		if(timertest_TimerIndex != 0)
		{
			...
			{
				DEBUGOUT("Test timer start,index :(%08X)",timertest_TimerIndex);
				
				testhandle_TimerIndex = SOFT_TIMER_CreateTimer(200,handle1,true);
				if(testhandle_TimerIndex != 0)
				{
					DEBUGOUT("handle1 timer create success :(%08X)",testhandle_TimerIndex);
					return true;
				}
				else
				{
					DEBUGOUT("handle1 timer create error");
					return false;
				}
			}
			else
			{
				DEBUGOUT("Test timer start error");
				return false;
			}
		}
		else
		{
			DEBUGOUT("Test timer create error");
			return false;
		}
	}
	else
	{
		...
		{
			if(FindNode(ActivTimerList,(SOFT_TIMER_LINK_NODE *)timertest_TimerIndex))
			{
				DEBUGOUT("Test timer activated already");
			}
			else
			{
				DEBUGOUT("Test timer inactivated");
			}
		}
		else
		{
			DEBUGOUT("Test timer Index error: Non-existent");
		}
		return false;
	}
}
